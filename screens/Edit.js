  ...
  const update = async () => {
    ...
    try {
      ...
      if(!name || !totalNoSeason){
        return alert("Please add the fields")
      }

   

      const storeValue =  await AsyncStorage.getItem('@season_list')
      const list = await JSON.parse(storeValue)
  ...
  useEffect(() => {
    ...
    return (
      ...
      <ScrollView contentContainerStyle={{flexGrow: 1}}>
          <H1 style={styles.heading}>Update watchlist</H1>
          <Form>
            ...
            <Button rounded block onPress={update} ><Text style={{color: "#fff"}}>Update</Text></Button>
          </Form>
      </ScrollView>
</Container>
    )
